...
class BtnLoadingProgressbar(view:View){
    ...
    fun setState(isSuccess:Boolean,onAnimationEnd:()->Unit){
        ...
        bgAnim.doOnEnd {
            if (isSuccess)
                flipProgressBar(R.drawable.ic_done,R.drawable.btn_loading_layout_success){ if (it) onAnimationEnd() }
            else
                flipProgressBar(R.drawable.ic_fail,R.drawable.btn_loading_layout_error){ if (it) onAnimationEnd() }
        }
    }
